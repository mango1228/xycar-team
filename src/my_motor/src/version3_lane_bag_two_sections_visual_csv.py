...
# -*- coding: utf-8 -*-
"""
version3_lane_bag_two_sections_visual_csv.py
...
실행 예시:
    ...
    python3 version3_lane_bag_two_sections_visual_csv.py \
...
class BaselineLaneDetector:
    ...
    def process(self, frame_bgr: np.ndarray) -> Dict[str, Any]:
        ...
        # 2) Version 3: 기본은 local-first. 단, local이 성공해도 full ROI가
        #    기하학적으로 타당하고 local 결과와 크게 다르면 prediction mismatch로 본다.
        can_try_local = (
            ...
            if local_ok:
                ...
                if full_valid:
                    ...
                    if mismatch:
                        ...
                        # full ROI 후보가 2프레임 이상 일관되면 stale local prediction보다 full ROI를 채택한다.
                        if tentative_count >= self.cfg.v3_tentative_accept_frames:
                            chosen = self._decorate_full_recheck_result(
                                full_result,
                                baseline_mode,
                                local_result,
                                decision="accept_full_after_tentative",
                                accept=True,
                            )
                            chosen["v3_tentative_count"] = tentative_count
                            return self._finalize_result(chosen)

                        # 아직 1프레임짜리 mismatch면 바로 갈아타지 않고 local을 유지하되 CSV에 기록한다.
                        local_result["v3_full_recheck_used"] = 1
                        local_result["v3_mismatch"] = 1
                        local_result["v3_tentative_count"] = tentative_count
                        local_result["v3_full_recheck_accept"] = 0
                        local_result["v3_decision"] = "hold_local_wait_full_consistency"
                        return self._finalize_result(local_result)

                # full ROI가 local과 크게 다르지 않거나 full 후보가 타당하지 않으면 local 결과를 채택한다.
                self._reset_tentative()
                local_result["v3_decision"] = "accept_local"
                return self._finalize_result(local_result)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="rosbag 한 개에서 코드 내부 TIME_SEGMENTS 구간만 버전 3 차선 검출 분석 + 구간별 CSV 저장"
    )
...
def handle_view(
    ...
    if view in ["overlay", "both"]:
        cv2.imshow("version2 section overlay - no save", resize_for_display(overlay, display_scale))

    if view in ["debug", "both"]:
        debug_grid = make_debug_grid(frame_bgr, overlay, result)
        cv2.imshow("version2 section debug - no save", resize_for_display(debug_grid, display_scale))
