...
# -*- coding: utf-8 -*-
"""
version1_lane_bag_two_sections_visual_csv.py
...
실행 예시:
    ...
    python3 version1_lane_bag_two_sections_visual_csv.py \
...
def draw_overlay(frame_bgr: np.ndarray, result: Dict[str, Any], cfg: BaselineConfig, segment_name: str, rel_time: float) -> np.ndarray:
    ...
    draw_text(overlay, "baseline={}  recovery_used={}  recovery_success={}  reason={}".format(
        result.get("baseline_mode", ""), result.get("recovery_used", 0),
        result.get("recovery_success", 0), result.get("recovery_reason", "")), (10, 125))
...
def handle_view(
    ...
    if view in ["overlay", "both"]:
        cv2.imshow("version1 section overlay - no save", resize_for_display(overlay, display_scale))

    if view in ["debug", "both"]:
        debug_grid = make_debug_grid(frame_bgr, overlay, result)
        cv2.imshow("version1 section debug - no save", resize_for_display(debug_grid, display_scale))
