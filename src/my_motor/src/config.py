...
class Config:
    def __init__(self):
        ...
        # 횡단보도 정지만 별도로 끄는 토글 (false면 횡단보도 감지해도 정지 안 함).
        # 빗금(사선 주차) 정지는 enable_special_zone 으로 별개 유지.
        self.enable_crosswalk_stop = rospy.get_param("~enable_crosswalk_stop", True)
        ...
        self.ar_stop_sec     = rospy.get_param("~ar_stop_sec", 0.0)      # 1차 정지(제거)
        self.ar_leftmost_sec = rospy.get_param("~ar_leftmost_sec", 1.0)  # 왼쪽 부채꼴 추종+ROI확대
        self.ar_stop2_sec    = rospy.get_param("~ar_stop2_sec", 0.0)     # 2차 정지(제거)
        ...
        self.ar_center_sec    = rospy.get_param("~ar_center_sec", 7.5)     # 중앙 최근접 부채꼴 추종
        self.ar_center_gain_scale = rospy.get_param("~ar_center_gain_scale", 2.0)  # 중앙 추종 게인 배율(장애물 회피 강화)
        ...
        self.ar_roi_forward_scale_center = rospy.get_param("~ar_roi_forward_scale_center", 2.2222)  # y_min -0.45*2.2222 = -1.00
        ...
        self.ar_after_center_sec = rospy.get_param("~ar_after_center_sec", 3.35)
        ...
        self.hatch_advance_sec     = rospy.get_param("~hatch_advance_sec", 1.8)
