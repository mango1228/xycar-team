...
# -*- coding: utf-8 -*-
"""
version1_local_roi_visual_csv.py

목적:
    하나의 rosbag에서 사용자가 코드 안에 지정한 여러 시간 구간만 골라서,
    버전 1 차선 검출 알고리즘을 검증한다.
...
실행 예시:
    ...
    python3 version1_local_roi_visual_csv.py \
...
TIME_SEGMENTS = [
    {
        "name": "lap1_problem_section_v1",
        "start_sec": 14.0,
        "end_sec": 80.0,
    },
    {
        "name": "lap2_problem_section_v1",
        "start_sec": 104.0,
        "end_sec": 115.0,
    },
]
...
class BaselineLaneDetector:
    ...
    동작 원리:
        1) 먼저 기존 baseline과 동일하게 full ROI에서 차선 검출을 수행한다.
        2) full ROI 결과가 BOTH/LEFT/RIGHT이면 그 결과를 그대로 사용한다.
        3) full ROI 결과가 NONE이고, 연속 실패 횟수가 recovery_fail_threshold 이상이면
           마지막으로 신뢰했던 last_good_lpos/rpos 주변 local ROI에서만
           CLAHE + adaptive Canny + HoughLinesP 복구를 시도한다.
        4) 복구 후보는 slope sign, last_good과의 위치 거리, lane width 일관성으로 검증한다.
    """
    ...
    def __init__(self, cfg: BaselineConfig):
        ...
        # Version 1 state: 신뢰도 높은 최근 검출 위치
    ...
    def _finalize_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if int(result.get("recovery_used", 0)) == 1:
            suspicious_reasons.append("v1_recovery_used")
        if int(result.get("recovery_success", 0)) == 1:
            suspicious_reasons.append("v1_recovery_success")
    ...
    def _detect_local_side(
        ...
        center_x: int,
        target_pos: int,
    ) -> Tuple[List[Tuple[int, int, int, int]], List[Tuple[int, int, int, int]], Optional[int], Optional[float], np.ndarray, Tuple[int, int]]:
        cfg = self.cfg
    ...
    def _attempt_recovery(self, full_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result["recovery_reason"] = "triggered_after_full_NONE"
        ...
        result.update({
            ...
            "pred_lpos": self.last_good_lpos,
            "pred_rpos": self.last_good_rpos,
            "final_source": "LOCAL_RECOVERY",
        })
        return result

    def process(self, frame_bgr: np.ndarray) -> Dict[str, Any]:
        # 1) baseline full ROI 검출 먼저 수행
        full_result = self._run_full_roi_detection(frame_bgr)

        # 2) full ROI가 성공하면 baseline 결과를 그대로 사용한다.
        if full_result["mode"] != "NONE":
            full_result["final_source"] = "FULL_BASELINE"
            return self._finalize_result(full_result)

        # 3) full ROI 실패가 누적됐을 때만 version1 local recovery를 시도한다.
        predicted_fail_count = self.fail_count + 1
        if predicted_fail_count >= self.cfg.recovery_fail_threshold:
            recovered_result = self._attempt_recovery(full_result)
            if int(recovered_result.get("recovery_success", 0)) == 1:
                recovered_result["final_source"] = recovered_result.get("final_source", "LOCAL_RECOVERY")
            else:
                recovered_result["final_source"] = "FULL_RECOVERY_FAILED"
            return self._finalize_result(recovered_result)

        full_result["recovery_used"] = 0
        full_result["recovery_success"] = 0
        full_result["recovery_reason"] = "below_fail_threshold"
        full_result["final_source"] = "FULL_BELOW_THRESHOLD"
        return self._finalize_result(full_result)
...
def draw_overlay(frame_bgr: np.ndarray, result: Dict[str, Any], cfg: BaselineConfig, segment_name: str, rel_time: float) -> np.ndarray:
    ...
    draw_text(overlay, "baseline={}  source={}  recovery={}/{}".format(
        result.get("baseline_mode", ""), result.get("final_source", "FULL"),
        result.get("recovery_used", 0), result.get("recovery_success", 0)), (10, 125))
    draw_text(overlay, "reason={}".format(
        result.get("recovery_reason", "")), (10, 150))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="rosbag 한 개에서 코드 내부 TIME_SEGMENTS 구간만 버전 1 차선 검출 분석 + 구간별 CSV 저장"
    )
    ...
    parser.add_argument("--recovery-fail-threshold", type=int, default=3,
                        help="full ROI mode=NONE이 이 횟수 이상 연속될 때 local recovery 시도")
...
def handle_view(
    ...
    if view in ["overlay", "both"]:
        cv2.imshow("version1 local ROI overlay - no save", resize_for_display(overlay, display_scale))

    if view in ["debug", "both"]:
        debug_grid = make_debug_grid(frame_bgr, overlay, result)
        cv2.imshow("version1 local ROI debug - no save", resize_for_display(debug_grid, display_scale))
            ...
            for topic, msg, stamp in bag.read_messages(topics=[args.topic]):
                ...
                for seg in active_segments:
                    ...
                    if seg.processed_count % 50 == 0:
                        seg.csv_file.flush()
                        print("[INFO] segment={} | processed={} | frame_idx={} | rel_t={:.3f}s | mode={} | baseline={} | rec={}/{} | center={} | roi_mean={:.1f} | hough={}".format(
                            seg.name,
                            seg.processed_count,
                            total_msg_count,
                            rel_time,
                            ...
                            result.get("recovery_used", 0),
                            result.get("recovery_success", 0),
                            result["center"],
                            result["roi_mean"],
