...
# -*- coding: utf-8 -*-
"""
baseline_lane_bag_visual_csv_only.py
...
def to_bgr_from_msg(bridge: CvBridge, msg: Any) -> np.ndarray:
    ...
    if isinstance(msg, CompressedImage):
        ...
        return image
    raise TypeError("지원하지 않는 메시지 타입: {}".format(type(msg)))
...
def draw_overlay(frame_bgr: np.ndarray, result: Dict[str, Any], cfg: BaselineConfig) -> np.ndarray:
    ...
    draw_text(overlay, "mode={}  center={}  err={}  fail={}".format(
        result["mode"], result["center"], result["center_error"], result["fail_count"]), (10, 25))
    draw_text(overlay, "roi_mean={:.1f}  edge={}  hough={}  L/R={}/{}".format(
        result["roi_mean"], result["edge_pixel_count"], result["hough_line_count"],
        result["left_line_count"], result["right_line_count"]), (10, 50))
        ...
        with rosbag.Bag(args.bag, "r") as bag:
            ...
            for topic, msg, stamp in bag.read_messages(topics=[args.topic]):
                ...
                try:
                    ...
                    print("[WARN] frame decode 실패: frame_idx={} | {}".format(total_msg_count, exc))
                    total_msg_count += 1
                    continue
                ...
                row = {
                    ...
                    "timestamp": "{:.9f}".format(stamp.to_sec()),
                ...
                if processed_count % 50 == 0:
                    csv_file.flush()
                    print("[INFO] processed={} | frame_idx={} | mode={} | center={} | roi_mean={:.1f} | hough={}".format(
                        processed_count,
                        total_msg_count,
                        result["mode"],
                        result["center"],
