...
# -*- coding: utf-8 -*-
"""
version2_local_roi_visual_csv.py

목적:
    하나의 rosbag에서 사용자가 코드 안에 지정한 여러 시간 구간만 골라서,
    버전 2 차선 검출 알고리즘을 검증한다.
...
실행 예시:
    ...
    python3 version2_local_roi_visual_csv.py \
...
# TIME_SEGMENTS = [
#     {
#         "name": "lap1_problem_section_v2",
#         "start_sec": 14.0,
#         "end_sec": 80.0,
#     },
#     {
#         "name": "lap2_problem_section_v2",
#         "start_sec": 104.0,
#         "end_sec": 115.0,
#     },
# ]
...
class BaselineConfig:
    ...
    # Version 2 local-first parameters
...
class BaselineLaneDetector:
    ...
    주의:
        Version 2는 prediction/last_good 기반 local-first 구조이므로,
    ...
    def __init__(self, cfg: BaselineConfig):
        ...
        # Version 2 state: 신뢰도 높은 최근 검출 위치
    ...
    def _finalize_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if int(result.get("recovery_used", 0)) == 1:
            suspicious_reasons.append("v2_local_first_used")
        if int(result.get("recovery_success", 0)) == 1:
            suspicious_reasons.append("v2_local_accept")
    ...
    def process(self, frame_bgr: np.ndarray) -> Dict[str, Any]:
        ...
        # 2) Version 2: last_good 기반 local ROI를 매 프레임 우선 탐색한다.
        can_try_local = (
        ...
        if can_try_local:
            ...
            full_result["local_left_roi"] = local_result.get("local_left_roi")
            full_result["local_right_roi"] = local_result.get("local_right_roi")
            full_result["pred_lpos"] = local_result.get("pred_lpos")
            full_result["pred_rpos"] = local_result.get("pred_rpos")
...
def draw_overlay(frame_bgr: np.ndarray, result: Dict[str, Any], cfg: BaselineConfig, segment_name: str, rel_time: float) -> np.ndarray:
    ...
    draw_text(overlay, "baseline={}  source={}  local={}/{}  fallback={}".format(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="rosbag 한 개에서 코드 내부 TIME_SEGMENTS 구간만 버전 2 차선 검출 분석 + 구간별 CSV 저장"
    )
...
def handle_view(
    ...
    if view in ["overlay", "both"]:
        cv2.imshow("version2 local ROI overlay - no save", resize_for_display(overlay, display_scale))

    if view in ["debug", "both"]:
        debug_grid = make_debug_grid(frame_bgr, overlay, result)
        cv2.imshow("version2 local ROI debug - no save", resize_for_display(debug_grid, display_scale))
